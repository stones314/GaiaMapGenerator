...
def get_hex_distances(hex_coords):
  ...
  for c1 in range(n-1):
    for c2 in range(c1+1,n):
      distances[i] = get_dist(hex_coords[c1],hex_coords[c2])#TODO: requires this function
      i = i + 1
  return distances
...
"""
  ...
  if n_rows < 1:
    return 0
  print "n_cols = " + str(n_cols) + ", n_rows = " + str(n_rows)
  for col in range(n_cols):
    for row in range(n_rows): #looping through the map
      if hex_map[col][row] == "Tr" or hex_map[col][row] == None:
        continue
      neighbours = get_hexes_at_radius(col,row,1)
      for i in range(2,4):#only look at neighbours "in front of" the planet, or we would check some stuff more than once
        ...
        if hex_map[col][row] == hex_map[neighbours[i][0]][neighbours[i][1]]:
          print hex_map[col][row] + " at + (" + str(col) +","+ str(row) +") == " + hex_map[neighbours[i][0]][neighbours[i][1]] + " at + (" + str(neighbours[i][0]) +","+ str(neighbours[i][1]) +")"
          print neighbours
          return 1
  return 0
...
def calc_happiness(planet_type,hex_map,GP,TP,HP,max_range):
  """
    ...
    for row in range(n_rows):
      if hex_map[col][row] == planet_type:
        for R in range(1,max_range):
          ...
          for coord in coords:
            if coord[0] < 0 or coord[0] >= n_cols:#TODO: these limit-checks can be included in get_hexes_at_range function instead?
              continue
            if coord[1] < 0 or coord[1] >= n_rows:
              continue
            if hex_map[coord[0]][coord[1]] == None:#TODO: the get_hexes function can return only hexes with a planet? or make another get_planets_at_range?
              continue
            if hex_map[coord[0]][coord[1]] == "Ga":
              ...
              happiness += HP[color_dist]/R
            n_data += 1
  return happiness/n_data
    ...
    for row in range(n_rows):
      ...
      while i < len(cluster_planets):
        ...
        if visited[planet_col][planet_row] == 1:
          continue #need to check this here since it might have been visited after it was added to cluster_planets
        ...
        for j in range(6):
          ...
          if (visited[neighbour_col][neighbour_row] == 1)           or (hex_map[neighbour_col][neighbour_row] in ignored_types):
...
class Map(object):
    def __init__(self, players, random=None):
        """
    ...
    def set_map(self):       
        ...
        if self.players == 2:
            ...
            self.centre = [[(6, 6), (11,7), (16, 8)], [(3, 13), (8,14), (13, 15), (18, 16)], [(5, 21), (10, 22), (15,23)]]
            if self.random == "Yes":
                random.shuffle(Large)
            self.content = Large

        self.map_data = copy.deepcopy(self.map)
        ...
        for j, row in enumerate(self.map):
            for i, item in enumerate(row):
                ...
                self.map_data[j][i] = (sector_number, sector_rotation)
                index +=1
    ...
    def make_image_map(self, sector_list, clockwise = True):
      """
    ...
    def update_map_data(self):
      """
      Update map with rotation of each sector
      """
      for j, row in enumerate(self.map):
            for i, sector in enumerate(row):
                sector_rotation = sector.get_rotation_deg()
                sector_number = self.map_data[j][i][0]
...
if __name__ == "__main__":
    ...
    test_map.rotate_map()
    #test_map.print_map()
    test_map.make_image_map(test_map.map_data)

    
    hex_map = test_map.get_map()
    #calc_happiness(planet_type,hex_map,GP,TP,HP,max_range): #returns a number
    happy = [0.0]*7
    for i in range(7):
      happy[i] = calc_happiness(color_wheel[i],hex_map,2.0,1.0,[2.5,1.5,1.0,1.0],3)
    print color_wheel
    print happy
    ...
    clusters.sort()
    print clusters
    test_map.map[0][0].rotate_sector_once()
